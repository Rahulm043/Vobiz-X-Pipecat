...
def get_call(call_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        res = (
            supabase.table("calls").select("*").eq("vobiz_call_uuid", call_id).execute()
        )
        if res.data:
            return res.data[0]
    except Exception as e:
...
def refresh_campaign_stats(campaign_id: str) -> Dict[str, int]:
    ...
    accounted = (
        stats["completed"] + stats["failed"] + stats["rejected"] + stats["active"]
    )
...
def get_agent_stats(
    ...
    try:
        ...
        total_minutes = sum(
            round_up_minutes(c.get("duration_seconds", 0) or 0) for c in filtered_calls
        )

        connected = sum(1 for c in filtered_calls if c.get("status") == "completed")
        failed = sum(
            1 for c in filtered_calls if c.get("status") in ("failed", "error")
        )
        rejected = sum(1 for c in filtered_calls if c.get("status") == "rejected")
        in_progress = sum(
            1
            for c in filtered_calls
            if c.get("status") in ("ringing", "connected", "in_progress")
        )
        ...
        for c in filtered_calls:
            ...
            breakdown[day]["minutes"] += round_up_minutes(
                c.get("duration_seconds", 0) or 0
            )
